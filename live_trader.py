...
# THE FIX: Boot up and load memory!
...
def run_bot_cycle():
    ...
    for order in manual_orders:
        ...
        try:
            ...
            if maction == "BUY" and msymbol not in trader.positions:
                ...
                trader.buy(msymbol, current_price, timestamp="MANUAL", amount_usdt=custom_amount, reason="Manual")
            elif maction == "SELL" and msymbol in trader.positions:
                print(f"⚡ EXECUTING MANUAL SELL: {msymbol}")
                trader.sell(msymbol, current_price, timestamp="MANUAL", reason="Manual")
    ...
    for symbol in SYMBOLS:
        try:
            ...
            trader.check_stop_loss_and_take_profit(symbol, latest_candle['close'], timestamp=timestamp)
